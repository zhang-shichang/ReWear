...
import { Camera, RefreshCw, Check, Plus, Loader2, Sparkles, Upload, Calendar, Edit2, X, Search } from 'lucide-react';
    ...
    <div className="flex h-[calc(100vh-64px)] w-full overflow-hidden bg-stone-900">
        ...
        {permissionError && !uploadedImage ? (
           <div className="text-center p-8 bg-stone-800 rounded-2xl max-w-md mx-4">
             ...
             <p className="text-stone-400 mb-6">
               Enable camera access to use the real-time detection feature. 
               If you've already granted permission, try refreshing or opening the app in a new tab.
             </p>
          ...
          <>
            ...
            {/* Scanning Overlay Effect - More subtle/artsy */}
      ...
      <div className="w-[420px] flex-shrink-0 bg-[#fafaf9] border-l border-stone-200 flex flex-col h-full relative z-10 shadow-2xl">
          ...
          {detectedItems.length === 0 && !isLogSuccess ? (
             <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
               ...
               <p className="text-stone-400 text-xs uppercase tracking-widest max-w-[200px] leading-relaxed">
                 Stand in frame or upload a photo to curate your daily entry
               </p>
               <button 
                 onClick={() => fileInputRef.current?.click()}
                      ...
                      <div className="flex gap-5 items-start">
                        ...
                        <div className="flex-1 min-w-0 pt-1">
                          {editingItemId === item.id ? (
                            ...
                            <>
                              <h4 
                                className="font-serif text-xl text-stone-900 italic cursor-pointer hover:text-primary-600 transition-colors"
        ...
        <div className="p-8 bg-[#fafaf9]">
          <button 
            disabled={detectedItems.length === 0 || isLogSuccess}
            onClick={handleLogOutfit}
            className={`
              w-full py-4 text-sm font-bold uppercase tracking-[0.15em] transition-all duration-500
              ${detectedItems.length > 0 && !isLogSuccess
                  ...
                  >
                    <div className="aspect-[3/4] overflow-hidden bg-stone-100 relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="bg-white px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-stone-900 shadow-xl">
                          Add to Look
                        </div>
                      </div>
                    </div>
                    <div>
                      <h4 className="font-serif text-sm italic text-stone-900 group-hover:text-primary-600 transition-colors truncate">
                        {item.name}
                      </h4>
