...
- Business logic: When a user clicks on the "Log Wear" button:
    ...
    + Optional: Have a button to revert the change if the user accidentally click on the button. 
- Next steps: 
    + Shichang: 
...
- An issue recorded by Professor is that the frontend is directly connected to the database. 
...
- Fix timeline: After merging frontend, backend, and database together to get a MVP. 
